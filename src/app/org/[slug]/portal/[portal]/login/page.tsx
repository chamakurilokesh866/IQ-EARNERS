...
import { rememberOrgPortalCode } from "@/lib/orgPortalClient"
...
export default function OrgPortalLoginPage() {
  const { slug, portal } = useParams<{ slug: string; portal: string }>()
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/org/${slug}/auth`, {
        ...
        body: JSON.stringify({ username: username.trim(), password, portalCode: portalNorm }),
      })
      const j = await res.json()
      if (j.ok) {
        rememberOrgPortalCode(String(slug), portalNorm)
        if (j.requiresPasswordReset) {
          router.replace(`/org/${slug}/change-password`)
          return
        }
        if ((j.role === "owner" || j.role === "admin") && j.requiresApiSetup) {
          router.replace(`/org/${slug}/setup-api-keys`)
        } else {
          router.replace(j.role === "owner" || j.role === "admin" ? `/org/${slug}/dashboard` : `/org/${slug}`)
        }
      } else {
  ...
  if (!org) {
    return (
      <div className="min-h-screen app-page-surface flex flex-col items-center justify-center gap-4 text-center px-6">
        ...
        <a href="/intro" className="mt-4 text-cyan-400 text-sm font-bold hover:underline">
          Go to IQ Earners main site
        </a>
      </div>
    )
  }
        ...
        <div className="w-full max-w-[420px]">
          <div className="text-center mb-8">
            {org.logo ? (
              ...
              <div
                className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center text-2xl font-black text-white border border-white/10"
                style={{ background: `${pc}30` }}
              >
                {org.name.charAt(0)}
              </div>
            )}
            ...
            <p className="mt-3 text-[10px] text-white/35 font-mono tracking-wider">Portal access ··· {portalNorm.slice(0, 4)}···{portalNorm.slice(-2)}</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-xl p-6 sm:p-8 shadow-2xl">
            <h2 className="text-lg font-black text-white mb-1">Secure organization sign in</h2>
            <p className="text-xs text-white/40 mb-6">Use the login link provided by your administrator. Session ends when you close the browser.</p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl bg-black/40 border border-white/10 px-4 py-3 text-sm text-white placeholder:text-white/25 focus:outline-none focus:ring-2 focus:border-transparent transition-all"
                  style={{ "--tw-ring-color": `${pc}50` } as React.CSSProperties}
            ...
            <button
              type="submit"
              disabled={submitting}
              className="mt-6 w-full rounded-xl py-3.5 text-sm font-black text-white transition-all hover:brightness-110 disabled:opacity-50"
              style={{ background: pc }}
            >
          ...
          <p className="text-center mt-8 text-[10px] text-white/25 uppercase tracking-widest font-bold">Powered by IQ Earners</p>
        </div>
      </div>
    </div>
  )
