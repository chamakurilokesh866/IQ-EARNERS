...
const STORAGE_PATH = "blocked-qr"
const FILE_PATH = path.join(process.cwd(), "public", "uploads", "blocked-qr", "blocked-qr")
...
export async function POST(req: Request) {
  ...
  const filename = `blocked-qr.${safeExt}`
  ...
  if (supabase) {
    try {
      ...
      const blockedQrUrl = urlData?.publicUrl ?? ""
      const ok = await updateSettings({ blockedQrUrl })
      if (!ok) return NextResponse.json({ ok: false, error: "Failed to save settings" }, { status: 500 })
      return NextResponse.json({ ok: true, blockedQrUrl })
    } catch (e) {
  ...
  try {
    ...
    const blockedQrUrl = `/uploads/blocked-qr/${filename}`
    const ok = await updateSettings({ blockedQrUrl })
    if (!ok) return NextResponse.json({ ok: false, error: "Failed to save settings" }, { status: 500 })
    return NextResponse.json({ ok: true, blockedQrUrl })
  } catch (e) {
