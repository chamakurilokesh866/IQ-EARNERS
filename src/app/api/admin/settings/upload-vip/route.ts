...
const STORAGE_PATH = "vip"
const FILE_PATH = path.join(process.cwd(), "public", "uploads", "vip", "vip-modal")
...
export async function POST(req: Request) {
    ...
    const safeExt = ["png", "jpg", "jpeg", "webp"].includes(ext) ? (ext === "jpeg" ? "jpg" : ext) : "png"
    const filename = `vip-promo.${safeExt}`
    ...
    if (supabase) {
        try {
            ...
            const vipModalImage = urlData?.publicUrl ?? ""
            const ok = await updateSettings({ vipModalImage })
            if (!ok) return NextResponse.json({ ok: false, error: "Failed to save settings" }, { status: 500 })
            return NextResponse.json({ ok: true, vipModalImage })
        } catch (e: any) {
    ...
    try {
        ...
        const vipModalImage = `/uploads/vip/${filename}`
        const ok = await updateSettings({ vipModalImage })
        if (!ok) return NextResponse.json({ ok: false, error: "Failed to save settings" }, { status: 500 })
        return NextResponse.json({ ok: true, vipModalImage })
    } catch (e: any) {
