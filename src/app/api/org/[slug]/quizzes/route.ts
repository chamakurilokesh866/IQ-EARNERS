...
import { createOrgQuiz, listOrgQuizzes, findOrgBySlug } from "@/lib/enterpriseStore"
...
export async function GET(_req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const visible = isOwnerOrAdmin ? quizzes : quizzes.filter((q) => q.published)
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const quiz = await createOrgQuiz(org.id, {
    ...
    createdBy: (auth.ok ? auth.session.memberId : "unknown"),
